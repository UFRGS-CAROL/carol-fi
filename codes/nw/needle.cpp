...
void WriteArrayFromFile(int* input_itemsets, int max_rows, char * filenameoutput) {
	int n = max_rows;// + 1;

	FILE *f_a;
	f_a = fopen(filenameoutput, "wb");
...
{
	fprintf(stderr, "Usage: %s <max_rows/max_cols> <penalty> <num_threads> <input_array> <output_file>\n", argv[0]);
...
runTest( int argc, char** argv) 
{
    ...
    WriteArrayFromFile(input_itemsets, max_rows, out_path);
