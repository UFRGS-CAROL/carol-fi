...
runTest( int argc, char** argv) 
{
    ...
    for( int i=1; i< max_rows ; i++){    //please define your own sequence. 
        input_itemsets[i*max_cols] = rand() % 24;
    }
    for( int j=1; j< max_cols ; j++){    //please define your own sequence.
        input_itemsets[j] = rand() % 24;
    }

    FILE *fpo;
    char input_name[500];
    snprintf(input_name, 500, "input_%d_th_%d_pen_%d",max_rows-1, omp_num_threads,penalty);
    //std::string input_name("input_" + std::to_string(max_rows-1)+"_threads_"+std::to_string(omp_num_threads));
    fpo = fopen(input_name, "wb");
    fwrite(input_itemsets, sizeof(int) * max_rows * max_cols, 1, fpo);
    fclose(fpo);
    ...
    snprintf(output_name, 500, "gold_%d_th_%d_pen_%d",max_rows-1, omp_num_threads, penalty);
    //std::string gold_name("gold_" + std::to_string(max_rows-1)+"_threads_"+std::to_string(omp_num_threads));
