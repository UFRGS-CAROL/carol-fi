...
* Usage:	./nn <filelist> <num> <target latitude> <target longitude> <out filename>
...
int main(int argc, char* argv[]) {
    ...
    if(argc < 6) {
        ...
        fprintf(stderr, "%s <filelist> <num neighbors to find> <target latitude> <target longitude> <out filename>\n",argv[0]);
        exit(-1);
    }
    ...
    FILE *file;
    if( (file = fopen(gold_filename, "wb" )) == 0 )
        printf( "The GOLD file was not opened\n" );
    for( j = 0 ; j < k ; j++ ) {
        if( !(neighbors[j].dist == OPEN) )
            fwrite(&neighbors[j],sizeof(struct neighbor), 1, file);
