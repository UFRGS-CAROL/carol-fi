...
void lud_diagonal_omp (double* a, int size, int offset)
{
    int i, j, k;
    for (i = 0; i < BS; i++) {
...
void lud_omp(double *a, int size)
{
...
#ifdef OMP_OFFLOAD
{
    ...
    for (offset = 0; offset < size - BS ; offset += BS)
    {
        ...
        for ( chunk_idx = 0; chunk_idx < chunks_in_inter_row; chunk_idx++)
        {
            ...
            double sum;           
            double temp[BS*BS] __attribute__ ((aligned (64)));
...
#pragma omp parallel for schedule(auto) default(none) \
        ...
        for  (chunk_idx =0; chunk_idx < chunks_per_inter; chunk_idx++)
        {
            ...
            double temp_top[BS*BS] __attribute__ ((aligned (64)));
            double temp_left[BS*BS] __attribute__ ((aligned (64)));
            double sum[BS] __attribute__ ((aligned (64))) = {0.f};
