...
int main( int argc, char *argv [])
{
    ...
    for(i=0; i<dim_cpu.space_elem; i=i+1) {
        ...
        fwrite(&(rv_cpu[i].v), 1, sizeof(float), file);
        fwrite(&(rv_cpu[i].x), 1, sizeof(float), file);
        fwrite(&(rv_cpu[i].y), 1, sizeof(float), file);
        fwrite(&(rv_cpu[i].z), 1, sizeof(float), file);
    }
    ...
    for(i=0; i<dim_cpu.space_elem; i=i+1) {
        ...
        fwrite(&(qv_cpu[i]), 1, sizeof(float), file);
    }
    fclose(file);
