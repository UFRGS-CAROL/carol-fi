...
void gpu_memory_unset(  int nstreams, int gpu_check,
        ...
        //=====================================================================
        for (int streamIdx = 0; streamIdx < nstreams; streamIdx++) {
                if (d_rv_gpu[streamIdx]) cudaFree(d_rv_gpu[streamIdx]);
                if (d_qv_gpu[streamIdx]) cudaFree(d_qv_gpu[streamIdx]);
                if (d_fv_gpu[streamIdx]) cudaFree(d_fv_gpu[streamIdx]);
                if (d_box_gpu[streamIdx]) cudaFree(d_box_gpu[streamIdx]);
        }
        if (gpu_check) {
                if (d_fv_gold_gpu) cudaFree(d_fv_gold_gpu);
        }
}
