...
void getParams(int argc, char** argv, int *boxes, int *generate,
		char **input_distances, char **input_charges, char **output_gold,
		int *iterations, int *verbose, int *fault_injection, int *nstreams) {
	if (argc < 2) {
		usage(argc, argv);
		exit(EXIT_FAILURE);
	}
	...
	*verbose = 0;
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "input_distances")) {
		...
		*input_distances = new char[100];
		snprintf(*input_distances, 100, "slava_distances_%i", *boxes);
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "input_charges")) {
		...
		*input_charges = new char[100];
		snprintf(*input_charges, 100, "slava_charges_%i", *boxes);
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "output_gold")) {
		...
		*output_gold = new char[100];
		snprintf(*output_gold, 100, "slava_gold_%i", *boxes);
	...
	if (checkCmdLineFlag(argc, (const char **) argv, "streams")) {
		*nstreams = getCmdLineArgumentInt(argc, (const char **) argv,
				"streams");
	}
...
__global__ void kernel_gpu_cuda(par_str d_par_gpu, dim_str d_dim_gpu,
		box_str* d_box_gpu, FOUR_VECTOR* d_rv_gpu, float* d_qv_gpu,
	...
	if (bx < d_dim_gpu.number_boxes) {
		...
		__shared__ float qB_shared[200];
		...
		for (k = 0; k < (1 + d_box_gpu[bx].nn); k++) {
			...
			while (wtx < NUMBER_PAR_PER_BOX) {
				...
				for (j = 0; j < NUMBER_PAR_PER_BOX; j++) {
					r2 =
							(float) rA_shared[wtx].v
									+ (float) rB_shared[j].v- DOT((float)rA_shared[wtx],(float)rB_shared[j]);
					u2 = a2 * r2;
					vij = exp(-u2);
					fs = 2 * vij;
					...
					fA[wtx].v += (float) ((float) qB_shared[j] * vij);
					fA[wtx].x += (float) ((float) qB_shared[j] * fxij);
					fA[wtx].y += (float) ((float) qB_shared[j] * fyij);
					fA[wtx].z += (float) ((float) qB_shared[j] * fzij);
				}
...
void generateInput(dim_str dim_cpu, char *input_distances, FOUR_VECTOR **rv_cpu,
		char *input_charges, float **qv_cpu) {
	...
	*rv_cpu = (FOUR_VECTOR*) malloc(dim_cpu.space_mem);
	for (i = 0; i < dim_cpu.space_elem; i = i + 1) {
		...
		(*rv_cpu)[i].v = (float) (rand() % 10 + 1) / 10.0;
		fwrite(&((*rv_cpu)[i].v), 1, sizeof(float), fp);
		// get a number in the range 0.1 - 1.0
		(*rv_cpu)[i].x = (float) (rand() % 10 + 1) / 10.0;
		fwrite(&((*rv_cpu)[i].x), 1, sizeof(float), fp);
		// get a number in the range 0.1 - 1.0
		(*rv_cpu)[i].y = (float) (rand() % 10 + 1) / 10.0;
		fwrite(&((*rv_cpu)[i].y), 1, sizeof(float), fp);
		// get a number in the range 0.1 - 1.0
		(*rv_cpu)[i].z = (float) (rand() % 10 + 1) / 10.0;
		fwrite(&((*rv_cpu)[i].z), 1, sizeof(float), fp);
	}
	fclose(fp);
	...
	*qv_cpu = (float*) malloc(dim_cpu.space_mem2);
	for (i = 0; i < dim_cpu.space_elem; i = i + 1) {
		// get a number in the range 0.1 - 1.0
		(*qv_cpu)[i] = (float) (rand() % 10 + 1) / 10.0;
		fwrite(&((*qv_cpu)[i]), 1, sizeof(float), fp);
	}
	fclose(fp);
}

void readInput(dim_str dim_cpu, char *input_distances, FOUR_VECTOR **rv_cpu,
		char *input_charges, float **qv_cpu, int fault_injection) {
	FILE *fp;
	int i;
	...
	*rv_cpu = (FOUR_VECTOR*) malloc(dim_cpu.space_mem);
	...
	for (i = 0; i < dim_cpu.space_elem; i = i + 1) {
		return_value[0] = fread(&((*rv_cpu)[i].v), 1, sizeof(float), fp);
		return_value[1] = fread(&((*rv_cpu)[i].x), 1, sizeof(float), fp);
		return_value[2] = fread(&((*rv_cpu)[i].y), 1, sizeof(float), fp);
		return_value[3] = fread(&((*rv_cpu)[i].z), 1, sizeof(float), fp);
	...
	*qv_cpu = (float*) malloc(dim_cpu.space_mem2);
	...
	for (i = 0; i < dim_cpu.space_elem; i = i + 1) {
		return_value[0] = fread(&((*qv_cpu)[i]), 1, sizeof(float), fp);
		if (return_value[0] == 0) {
...
void readGold(dim_str dim_cpu, char *output_gold, FOUR_VECTOR **fv_cpu_GOLD) {
	...
	*fv_cpu_GOLD = (FOUR_VECTOR*) malloc(dim_cpu.space_mem);
	...
	for (i = 0; i < dim_cpu.space_elem; i = i + 1) {
		return_value[0] = fread(&((*fv_cpu_GOLD)[i].v), 1, sizeof(float), fp);
		return_value[1] = fread(&((*fv_cpu_GOLD)[i].x), 1, sizeof(float), fp);
		return_value[2] = fread(&((*fv_cpu_GOLD)[i].y), 1, sizeof(float), fp);
		return_value[3] = fread(&((*fv_cpu_GOLD)[i].z), 1, sizeof(float), fp);
		if (return_value[0] == 0 || return_value[1] == 0 || return_value[2] == 0
				|| return_value[3] == 0) {
			printf("error reading rv_cpu from file\n");
#ifdef LOGS
...
void writeGold(dim_str dim_cpu, char *output_gold, FOUR_VECTOR **fv_cpu) {
	...
	for (i = 0; i < dim_cpu.space_elem; i = i + 1) {
		...
		fwrite(&((*fv_cpu)[i].v), 1, sizeof(float), fp);
		fwrite(&((*fv_cpu)[i].x), 1, sizeof(float), fp);
		fwrite(&((*fv_cpu)[i].y), 1, sizeof(float), fp);
		fwrite(&((*fv_cpu)[i].z), 1, sizeof(float), fp);
	}
	fclose(fp);
}
...
int main(int argc, char *argv[]) {
	...
	getParams(argc, argv, &dim_cpu.boxes1d_arg, &generate, &input_distances,
			&input_charges, &output_gold, &iterations, &verbose,
			&fault_injection, &nstreams);

	char test_info[200];
	snprintf(test_info, 200,
			"type:single-precision streams:%d boxes:%d block_size:%d", nstreams,
			dim_cpu.boxes1d_arg, NUMBER_THREADS);
	printf("%s\n", test_info);
#ifdef LOGS
	if (!generate) start_log_file("cudaSLavaMD", test_info);
#endif
	...
	for (loop = 0; loop < iterations; loop++) {
		...
		for (streamIdx = 0; streamIdx < nstreams; streamIdx++) {
...
#ifdef SAFE_MALLOC
			safe_cuda_malloc_cover((void **) &(d_box_gpu[streamIdx]),
					dim_cpu.box_mem);
#else
			cuda_error = cudaMalloc((void **) &(d_box_gpu[streamIdx]),
					dim_cpu.box_mem);
#endif
			...
			//==================================================
#ifdef SAFE_MALLOC
			safe_cuda_malloc_cover((void **) &(d_rv_gpu[streamIdx]),
					dim_cpu.space_mem);
#else
			cuda_error = cudaMalloc((void **) &(d_rv_gpu[streamIdx]),
					dim_cpu.space_mem);
#endif
			error_string = cudaGetErrorString(cuda_error);
			if (strcmp(error_string, "no error") != 0) {
			...
			//==================================================
#ifdef SAFE_MALLOC
			safe_cuda_malloc_cover((void **) &(d_qv_gpu[streamIdx]),
					dim_cpu.space_mem2);
#else
			cuda_error = cudaMalloc((void **) &(d_qv_gpu[streamIdx]),
					dim_cpu.space_mem2);
#endif
			error_string = cudaGetErrorString(cuda_error);
			if (strcmp(error_string, "no error") != 0) {
			...
			//==================================================
#ifdef SAFE_MALLOC
			safe_cuda_malloc_cover((void **) &(d_fv_gpu[streamIdx]),
					dim_cpu.space_mem);
#else
			cuda_error = cudaMalloc((void **) &(d_fv_gpu[streamIdx]),
					dim_cpu.space_mem);
#endif
			error_string = cudaGetErrorString(cuda_error);
			if (strcmp(error_string, "no error") != 0) {
			...
			cuda_error = cudaMemcpy(d_box_gpu[streamIdx], box_cpu,
					dim_cpu.box_mem, cudaMemcpyHostToDevice);
			error_string = cudaGetErrorString(cuda_error);
			if (strcmp(error_string, "no error") != 0) {
			...
			cuda_error = cudaMemcpy(d_rv_gpu[streamIdx], rv_cpu,
					dim_cpu.space_mem, cudaMemcpyHostToDevice);
			error_string = cudaGetErrorString(cuda_error);
			if (strcmp(error_string, "no error") != 0) {
			...
			cuda_error = cudaMemcpy(d_qv_gpu[streamIdx], qv_cpu,
					dim_cpu.space_mem2, cudaMemcpyHostToDevice);
			error_string = cudaGetErrorString(cuda_error);
			if (strcmp(error_string, "no error") != 0) {
			...
			cuda_error = cudaMemcpy(d_fv_gpu[streamIdx], fv_cpu,
					dim_cpu.space_mem, cudaMemcpyHostToDevice);
			error_string = cudaGetErrorString(cuda_error);
			if (strcmp(error_string, "no error") != 0) {
		...
		if (verbose)
			printf("[Iteration #%i] Setup prepare time: %.4fs\n", loop,
					mysecond() - timestamp);
		...
		//=====================================================================
		if (generate) {
			cuda_error = cudaMemcpy(fv_cpu, d_fv_gpu[0], dim_cpu.space_mem,
			...
			for (streamIdx = 0; streamIdx < nstreams; streamIdx++) {
				...
				cuda_error = cudaMemcpy(fv_cpu, d_fv_gpu[streamIdx],
...
#pragma omp critical
						{
							...
							snprintf(error_detail, 300,
									"stream: %d, p: [%d], ea: %d, v_r: %1.16e, v_e: %1.16e, x_r: %1.16e, x_e: %1.16e, y_r: %1.16e, y_e: %1.16e, z_r: %1.16e, z_e: %1.16e\n",
									streamIdx, i, thread_error, fv_cpu[i].v,
									fv_cpu_GOLD[i].v, fv_cpu[i].x,
									fv_cpu_GOLD[i].x, fv_cpu[i].y,
									fv_cpu_GOLD[i].y, fv_cpu[i].z,
									fv_cpu_GOLD[i].z);
							if (kernel_errors < 25)
								printf("ERROR: %s\n", error_detail);
							if (kernel_errors >= 25)
			...
			if (verbose)
				printf("[Iteration #%i] Gold check time: %f\n", loop,
						mysecond() - timestamp);
		}
		...
		if (verbose)
			printf(
					"[Iteration #%i] BOXES:%d BLOCK:%d OUTPUT/S:%.2f FLOPS:%.2f (GFLOPS:%.2f)\n",
					loop, dim_cpu.boxes1d_arg, NUMBER_THREADS, outputpersec,
					flops, flops / 1000000000);
		if (verbose)
			printf("[Iteration #%i] kernel_time:%f\n", loop, kernel_time);
		//=====================
