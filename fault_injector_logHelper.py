...
def finish(section):
    ...
    if checkIsOutput(section):
        p = os.popen('grep END /var/radiation-benchmarks/log/*|wc -l')
        s = int(p.readline())
        p.close()
        if s == 0:
            isHang = True
...
def checkIsOutput(section):
    proc = subprocess.Popen("ls /var/radiation-benchmarks/log/", stdout=subprocess.PIPE, shell=True)
    (out,err)=proc.communicate()
    if re.search("log",str(out)):
        return True
    else:
        return False
...
def saveOutput(section, isHang):
    ...
    if isOutput:
        subprocess.call("mv /var/radiation-benchmarks/log/* "+cpDir, shell=True)
    subprocess.call("rm -f /var/radiation-benchmarks/log/*", shell=True)
...
def checkSDCs(section):
    p = os.popen('grep SDC /var/radiation-benchmarks/log/*|wc -l')
    s = int(p.readline())
    p.close()
    if s > 0:
        return True
